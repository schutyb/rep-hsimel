...
def add_color_wheel(axis, num_colors=1024, saturation=0.8, value=0.95,
    ...
    hue_start = max(0, min(hue_start, 360))
    hue_end = max(0, min(hue_end, 360))
    display_start = max(0, min(display_start, 360))
    display_end = max(0, min(display_end, 360))

    # Rescale hue range to match display range
    hues = np.linspace(hue_start, hue_end, num_colors)
    hues = np.interp(hues, (hue_start, hue_end), (display_start, display_end))

    # Define the HSV values for the color wheel
    saturation = np.linspace(saturation, saturation, num_colors)
    value = np.linspace(value, value, num_colors)

    # Add the color wheel to the plot
    for i in range(num_colors):
        wedge_start = hues[i]
        wedge_end = hues[(i + 1) % num_colors]
        if wedge_start >= display_start and wedge_end <= display_end:
            axis.add_patch(Wedge((0, 0), rad_cutoff, wedge_start, wedge_end,
                                 width=rad_cutoff - 0.85, color=hsv_to_rgb((i / num_colors, saturation[i], value[i]))))
...
def concatenate(im, m, n, hper=0.05, vper=0.05, bidirectional=False):
    """
        ...
        for i in range(hsv.shape[0]):
            for j in range(hsv.shape[1]):
                if ph[i][j] == 0:
                    ...
                    if outlier_cut:
                        if (phinterval[0] <= ph[i][j] <= phinterval[1]) and (
                                mdinterval[0] <= md[i][j] <= mdinterval[1]):
                            hsv[i][j][0] = color_scale * (ph[i][j] - phinterval[0]) / abs(phinterval[0] - phinterval[1])
...
def histt(dc, g, s, imin, imax=None):
    """
    Use this function to filter the background deleting, those pixels where the intensity value is under ic.
