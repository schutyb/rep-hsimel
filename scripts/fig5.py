...
if plot:
    ...
    coordx, coordy = np.zeros(len(names)), np.zeros(len(names))
    for k in range(len(names)):
        ...
        coordx[k], coordy[k] = cluster.cluster_centers_[0][0], cluster.cluster_centers_[0][1]

        circle1 = plt.Circle((coordx[k], coordy[k]), 0.02, color=cols[k])
        ...
        ax3.grid()
    # df = pd.DataFrame(list(zip(*[coordx, coordy]))).add_prefix('Col')
    # df.to_csv('/home/bruno/Documentos/Proyectos/hsimel/Paper/figures/fig5/cm_components.csv', index=False)
    plt.show()
