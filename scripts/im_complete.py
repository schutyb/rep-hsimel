...
if cuarto:
    ...
    ax1.plot(binsmd[1:100], histn[0][1:] / max(histn[0][1:]), 'r', label='Nevus')
    ax1.plot(binsmd[1:100], histnp[0][1:] / max(histnp[0][1:]), 'b', label='Pigmented nevus')
    ax1.plot(binsmd[1:100], histm[0][1:] / max(histm[0][1:]), 'k', label='Melanoma')
    ax1.set_yscale('log')
    ax1.set_xlabel('Modulation')
    ax1.grid()
    ax1.legend()

    ax2.plot(bins[0:134], histnph[0] / max(histnph[0]), 'r', label='Nevus')
    ax2.plot(bins[0:134], histnpph[0] / max(histnpph[0]), 'b', label='Pigmented nevus')
    ax2.plot(bins[0:134], histmph[0] / max(histmph[0]), 'k', label='Melanoma')
    ax2.set_yscale('log')
