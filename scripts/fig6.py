...
figsup = True  # figura suplementaria de los espectros promedios
if figsup:
    hsi_stack = np.asarray(imn)
    hsipy.hsi_visualization.interactive1(dcn, gn, sn, 0.13, nbit=8, ncomp=4, filt=True, nfilt=3, spectrums=True,
                                         hsi_stack=hsi_stack, lamd=np.arange(418, 718, 10))

fig6 = False  # figura 6 del paper
if fig6:
    # filtreo y umbralizo
    for k in range(5):
        dcn = median(dcn)
